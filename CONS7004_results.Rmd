...
## Difference between SST and TF2 (*diff*) {.tabset}
...
mu0       <- 10
sigma0   <- 2
nu0      <- 2  # For SST, 1 = symmetrical, > 1 = right skewed, < 1 = left skewed
...
```{r}
SEP3model <- gamlss(y ~ 1, sigma.fo = ~ 1, nu.fo = ~ 1, tau.fo = ~ 1, family = SEP3(), data = dat, trace = FALSE)

AIC(SEP3model)

SHINmodel <- gamlss(y ~ 1, sigma.fo = ~ 1, nu.fo = ~ 1, tau.fo = ~ 1, family = SHASHo2(), data = dat, trace = FALSE)
...
## Difference between SST and TF2 (*same*) {.tabset}

<br> 

### Model

```{r, make them the same}
# both slightly skewed and slightly heavy tails
y <- rSST(n, mu = mu0, sigma = sigma0, nu = 1.2, tau = 5)
y1 <- rTF2(n, mu = mu0, sigma = sigma0, nu = 7)
